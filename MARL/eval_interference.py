...
def eval_policy(args, overlap_prob):
    ...
    env.config["observation"].update({"overlap_prob": overlap_prob})
    ...
    for i in t:
        ...
        if args.initial_pos != "":
            ...
            env.road.vehicles = load_veh.tolist()
            env.road.vehicles = np.delete(load_veh, 2).tolist()
            # env.road.vehicles = np.delete(load_veh, 12).tolist()
        ...
        while not (done or truncated):
            ...
            # print(env.road.vehicles[1].speed)
        ...
        # t.set_description(f"Crashrate {crashes/(j+1)} Mergerate {sucessfull_merges/(j+1)} Speed {speed/total_steps}")
        t.set_description(
            f"Crashrate {crashes/(j+1)} Other crashes {other_crashes/(j+1)}"
        )
    ...
    # f = open("overlap_prob.txt", "a")
    # f.write(f"{overlap_prob}, {crashes/num_tries}")
    # f.close()
