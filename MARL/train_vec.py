...
def train(args):
    ...
    #env_seq = gym.make('merge-single-agent-v0')
    #env_seq.config.update(config['env_config'])
    env = make_vec_env('merge-single-agent-v0', n_envs=4, vec_env_cls=SubprocVecEnv)
    old_config = env.get_attr("config")[0]
    old_config.update(config['env_config'])
    env.env_method("set_config", old_config)

    #env_seq.road.initial_vehicles = deepcopy(env.get_attr("road")[0].initial_vehicles)
    ...
    #env.reset()
    #env_seq.reset()
    #print("now stepping parallel env")
    #obs, reward, term,  info = env.step([0])
    #print(obs)
    #print("now stepping sequential env")
    #obs_seq, reward_seq, term_seq, trunc_seq, info_seq = env_seq.step(0)
    #print(obs_seq)
    #exit(0)
    ...
    if curriculum_learning == True:
        ...
        model.learn(int(5e5), tb_log_name=args.exp_tag + f"_seed_{seed_}", reset_num_timesteps=False, callback=callback_list)

        old_config["traffic_density"] = 3
        env.env_method("set_config", old_config)

        #env.config['traffic_density'] = 3
        model.learn(int(6e5), tb_log_name=args.exp_tag + f"_seed_{seed_}", reset_num_timesteps=False, callback=callback_list)
